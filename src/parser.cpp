...
private:
    ...
    std::unique_ptr<ASTNode> parseVariableDeclaration() {
        ...
        return std::make_unique<VariableDeclarationNode>(nameToken.value, std::move(typeNode));
    }
    ...
    std::unique_ptr<ASTNode> parseExpression() {
        ...
        //         left = std::make_unique<BinaryExpressionNode>(std::move(left), operatorToken.data.operatorType, std::move(right));
        //     } else {
    ...
    //     if (peek().general_type == TokenType::OPERATOR && peek().data.operatorType == OperatorType::OPEN_BRACE) {
    //         consume(TokenType::OPERATOR); //expect '{'

    //         std::vector<std::unique_ptr<ASTNode>> statements;

    //         while (peek().general_type != TokenType::OPERATOR || peek().data.operatorType != OperatorType::CLOSE_BRACE) {
    ...
    //     if (peek().general_type == TokenType::KEYWORD && peek().value == "else") {
    //         consume(TokenType::KEYWORD); //Consume ELSE
    //         elseBranch = parseBlock();
    //     }

    //     return std::make_unique<IfStatementNode>(std::move(condition), std::move(trueBranch), std::move(elseBranch));
    // }
...
extern void map_nodes() {
    std::map<std::string, NodeCreator> int_nodes {
        {"int", [](const Token& token) -> std::unique_ptr<ASTNode> {
            return std::make_unique<IntNode>(std::stoi(token.value));
        }},
        {"i8", [](const Token& token) -> std::unique_ptr<ASTNode> {
            return std::make_unique<I8Node>(std::stoi(token.value));
        }},
        {"i16", [](const Token& token) -> std::unique_ptr<ASTNode> {
            return std::make_unique<I16Node>(std::stoi(token.value));
        }},
        {"i32", [](const Token& token) -> std::unique_ptr<ASTNode> {
            return std::make_unique<I32Node>(std::stoi(token.value));
        }},
        {"i64", [](const Token& token) -> std::unique_ptr<ASTNode> {
            return std::make_unique<I64Node>(std::stoi(token.value));
        }},
        {"uint", [](const Token& token) -> std::unique_ptr<ASTNode> {
            return std::make_unique<UIntNode>(std::stoi(token.value));
        }},
        {"u8", [](const Token& token) -> std::unique_ptr<ASTNode> {
            return std::make_unique<U8Node>(std::stoi(token.value));
        }},
        {"u16", [](const Token& token) -> std::unique_ptr<ASTNode> {
            return std::make_unique<U16Node>(std::stoi(token.value));
        }},
        {"u32", [](const Token& token) -> std::unique_ptr<ASTNode> {
            return std::make_unique<U32Node>(std::stoi(token.value));
        }},
        {"u64", [](const Token& token) -> std::unique_ptr<ASTNode> {
...
int main() {
    ...
    for (const auto& code : testInputs) {
        ...
        for (const auto& token : tokens) {
            std::printf("Token: %d, Value %s\n", static_cast<int>(token.general_type), token.value.c_str());
        }
